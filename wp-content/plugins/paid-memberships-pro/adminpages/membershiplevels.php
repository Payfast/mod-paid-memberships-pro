...
if ($action == "save_membershiplevel") {
    ...
    if ($saveid > 0) {
        ...
        $sqlQuery = " INSERT INTO {$wpdb->pmpro_membership_levels}
						...
						VALUES
						( '" . esc_sql($ml_name) . "', '" . esc_sql($ml_description) . "', '" . esc_sql(
                $ml_confirmation
            ) . "', '" . esc_sql($ml_initial_payment) . "', '" . esc_sql($ml_billing_amount) . "', '" . esc_sql(
                        $ml_cycle_number
                    ) . "', '" . esc_sql($ml_cycle_period) . "', '" . esc_sql($ml_billing_limit) . "', '" . esc_sql(
                        $ml_trial_amount
                    ) . "', '" . esc_sql($ml_trial_limit) . "', '" . esc_sql($ml_expiration_number) . "', '" . esc_sql(
                        $ml_expiration_period
                    ) . "', '" . esc_sql($ml_allow_signups) . "' )";
        $wpdb->query($sqlQuery);
        if (empty($wpdb->last_error)) {
    ...
    if ($ml_id > 0) {
        ...
        $user_ids = $wpdb->get_col(
            "SELECT user_id FROM $wpdb->pmpro_memberships_users WHERE membership_id = '$ml_id' AND status = 'active'"
        );
        foreach ($user_ids as $user_id) {
            ...
            if (pmpro_changeMembershipLevel(0, $user_id)) {
                ...
                $pmproemail->data = array(
                    "body" => "<p>" . sprintf(
                            __(
                                "There was an error canceling the subscription for user with ID=%d. You will want to check your payment gateway to see if their subscription is still active.",
                                "pmpro"
                            ),
                            $user_id
                        ) . "</p>"
                ...
                $last_order       = $wpdb->get_row(
                    "SELECT * FROM $wpdb->pmpro_membership_orders WHERE user_id = '" . $user_id . "' ORDER BY timestamp DESC LIMIT 1"
                );
                if ($last_order) {
                    $pmproemail->data["body"] .= "<p>" . __("Last Invoice", "pmpro") . ":<br />" . nl2br(
                            var_export($last_order, true)
                        ) . "</p>";
                }
...
if ($edit) {
    ...
    <div>
        ...
        if (!empty($edit) && $edit > 0) {
            $level   = $wpdb->get_row(
                "SELECT * FROM $wpdb->pmpro_membership_levels WHERE id = '$edit' LIMIT 1",
                OBJECT
            );
            $temp_id = $level->id;
        } elseif (!empty($copy) && $copy > 0) {
            $level     = $wpdb->get_row(
                "SELECT * FROM $wpdb->pmpro_membership_levels WHERE id = '$copy' LIMIT 1",
                OBJECT
            );
            $temp_id   = $level->id;
            $level->id = null;
        } else// didn't find a membership level, let's add a new one...
            if (empty($level)) {
                $level                    = new stdClass();
                $level->id                = null;
                $level->name              = null;
        ...
        <form action="" method="post" enctype="multipart/form-data">
            ...
            <table class="form-table">
                ...
                <tr>
                    <th scope="row" valign="top"><label for="name"><?php
                            _e('Name', 'pmpro'); ?>:</label></th>
                    <td><input name="name" type="text" size="50" value="<?php
                        echo esc_attr($level->name); ?>"/></td>
                </tr>

                <tr>
                    <th scope="row" valign="top"><label for="description"><?php
                            _e('Description', 'pmpro'); ?>:</label></th>
                    <td>
                        <div id="poststuff" class="pmpro_description">
                            ...
                            if (version_compare($wp_version, "3.3") >= 0) {
                                ...
                                ?>
                                <textarea rows="10" cols="80" name="description" id="description"><?php
                                    echo esc_textarea($level->description); ?></textarea>
                                <?php
                            }
                            ?>
                        </div>
                ...
                <tr>
                    <th scope="row" valign="top"><label for="confirmation"><?php
                            _e('Confirmation Message', 'pmpro'); ?>:</label></th>
                    <td>
                        <div class="pmpro_confirmation">
                            ...
                            if (version_compare($wp_version, "3.3") >= 0) {
                                ...
                                ?>
                                <textarea rows="10" cols="80" name="confirmation" id="confirmation"><?php
                                    echo esc_textarea($level->confirmation); ?></textarea>
                                <?php
                            }
                            ?>
                        </div>
            ...
            <h3 class="topborder"><?php
                _e('Billing Details', 'pmpro'); ?></h3>
            <table class="form-table">
                <tbody>
                <tr>
                    <th scope="row" valign="top"><label for="initial_payment"><?php
                            _e('Initial Payment', 'pmpro'); ?>:</label></th>
                    <td>
                        <?php
                        ...
                        echo esc_attr($level->initial_payment); ?>"/>
                        <?php
                        ...
                        ?>
                        <small><?php
                            _e('The initial amount collected at registration.', 'pmpro'); ?></small></td>
                </tr>

                <tr>
                    <th scope="row" valign="top"><label><?php
                            _e('Recurring Subscription', 'pmpro'); ?>:</label></th>
                    <td><input id="recurring" name="recurring" type="checkbox" value="yes" <?php
                        ...
                        <label for="recurring"><?php
                            _e('Check if this level has a recurring subscription payment.', 'pmpro'); ?></label></td>
                </tr>

                <tr class="recurring_info" <?php
                if (!pmpro_isLevelRecurring($level)) { ?>style="display: none;"<?php
                } ?>>
                    <th scope="row" valign="top"><label for="billing_amount"><?php
                            _e('Billing Amount', 'pmpro'); ?>:</label></th>
                    <td>
                        <?php
                        if (pmpro_getCurrencyPosition() == "left") {
                            echo $pmpro_currency_symbol;
                        ...
                        echo esc_attr($level->billing_amount); ?>"/>
                        <?php
                        if (pmpro_getCurrencyPosition() == "right") {
                            echo $pmpro_currency_symbol;
                        }
                        ...
                        if ($gateway == "payfast")
                        { ?>
                        <input id="cycle_number" name="cycle_number" type="hidden" size="10" value="1"/>
                        ...
                        echo esc_attr($level->cycle_number); ?>"/>
                            <select id="cycle_period" name="cycle_period">
                                <?php
                                }

                                if ($gateway = "payfast") {
                                    $cycles = array(__('Month', 'pmpro') => 'Month', __('Year', 'pmpro') => 'Year');
                                } else {
                                    $cycles = array(
                                        __('Day(s)', 'pmpro')   => 'Day',
                                        __('Week(s)', 'pmpro')  => 'Week',
                                        __('Month(s)', 'pmpro') => 'Month',
                                        __('Year(s)', 'pmpro')  => 'Year'
                                    );
                                }
                            ...
                            <br/><small>
                                <?php
                                _e('The amount to be billed one cycle after the initial payment.', 'pmpro'); ?>
                                <?php
                                if ($gateway == "stripe") { ?>
                                <br/><strong <?php
                                             if (!empty($pmpro_stripe_error)) { ?>class="pmpro_red"<?php
                                } ?>><?php
                                    _e(
                                        'Stripe integration currently only supports billing periods of "Week", "Month" or "Year".',
                                        'pmpro'
                                    ); ?>
                                    <?php
                                    } elseif ($gateway == "braintree") { ?>
                                    <br/><strong <?php
                                                 if (!empty($pmpro_braintree_error)) { ?>class="pmpro_red"<?php
                                    } ?>><?php
                                        _e(
                                        ...
                                        <br/><strong <?php
                                                     if (!empty($pmpro_payflow_error)) { ?>class="pmpro_red"<?php
                                        } ?>><?php
                                            _e(
                                                'Payflow integration currently only supports billing frequencies of 1 and billing periods of "Week", "Month" or "Year".',
                                                'pmpro'
                                            ); ?>
                                            <?php
                                            } ?>
                            ...
                            if ($gateway == "braintree" && $edit < 0) { ?>
                                ...
                                <p class="pmpro_message"><strong><?php
                                        _e('Note', 'pmpro'); ?>:</strong> <?php
                                    _e(
                                        'You will need to create a "Plan" in your Braintree dashboard with the same settings and the "Plan ID" set to',
                                        'pmpro'
                                    ); ?> <em>pmpro_<?php
                                        echo $level->id; ?></em>.</p>
                                <?php
                ...
                if (!pmpro_isLevelRecurring($level)) { ?>style="display: none;"<?php
                    ...
                    <th scope="row" valign="top"><label for="billing_limit"><?php
                            _e('Billing Cycle Limit', 'pmpro'); ?>:</label></th>
                    <td>
                        ...
                        <br/><small>
                            ...
                            if ($gateway == "stripe") { ?>
                                <br/><strong <?php
                                    ...
                                    _e(
                                        'Stripe integration currently does not support billing limits. You can still set an expiration date below.',
                                        'pmpro'
                                    ); ?></strong>
                                <?php
                            } ?>
                ...
                if ($gateway != "payfast") { ?>
                    ...
                    if (!pmpro_isLevelRecurring($level)) {
                        ...
                        <th scope="row" valign="top"><label><?php
                                _e('Custom Trial', 'pmpro'); ?>:</label></th>
                        <td>
                            <input id="custom_trial" name="custom_trial" type="checkbox" value="yes" <?php
                            if (pmpro_isLevelTrial($level)) {
                                echo "checked='checked'";
                            } ?> onclick="jQuery('.trial_info').toggle();"/> <label for="custom_trial"><?php
                                _e('Check to add a custom trial period.', 'pmpro'); ?></label>

                            <?php
                            if ($gateway == "twocheckout") { ?>
                                <br/><small><strong <?php
                                                    if (!empty($pmpro_twocheckout_error)) { ?>class="pmpro_red"<?php
                                    } ?>><?php
                                        _e(
                    ...
                    if (!pmpro_isLevelTrial($level)) {
                        ...
                        <th scope="row" valign="top"><label for="trial_amount"><?php
                                _e('Trial Billing Amount', 'pmpro'); ?>:</label></th>
                        <td>
                            <?php
                            if (pmpro_getCurrencyPosition() == "left") {
                                echo $pmpro_currency_symbol;
                            ...
                            echo esc_attr($level->trial_amount); ?>"/>
                            <?php
                            if (pmpro_getCurrencyPosition() == "right") {
                                echo $pmpro_currency_symbol;
                            }
                            ?>
                            <small><?php
                                _e('for the first', 'pmpro'); ?></small>
                            <input name="trial_limit" type="text" size="10" value="<?php
                            echo esc_attr($level->trial_limit); ?>"/>
                            <small><?php
                                _e('subscription payments', 'pmpro'); ?>.</small>
                            <?php
                            if ($gateway == "stripe") { ?>
                                <br/><small>
                                    <strong <?php
            ...
            <h3 class="topborder"><?php
                _e('Other Settings', 'pmpro'); ?></h3>
            <table class="form-table">
                <tbody>
                <tr>
                    <th scope="row" valign="top"><label><?php
                            _e('Disable New Signups', 'pmpro'); ?>:</label></th>
                    <td><input id="disable_signups" name="disable_signups" type="checkbox" value="yes" <?php
                        if ($level->id && !$level->allow_signups) { ?>checked="checked"<?php
                        } ?> /> <label for="disable_signups"><?php
                            _e(
                ...
                <tr>
                    <th scope="row" valign="top"><label><?php
                            _e('Membership Expiration', 'pmpro'); ?>:</label></th>
                    <td><input id="expiration" name="expiration" type="checkbox" value="yes" <?php
                        ...
                        <label for="expiration"><?php
                            _e('Check this to set when membership access expires.', 'pmpro'); ?></label></a></td>
                </tr>

                <tr class="expiration_info" <?php
                if (!pmpro_isLevelExpiring($level)) { ?>style="display: none;"<?php
                } ?>>
                    <th scope="row" valign="top"><label for="billing_amount"><?php
                            _e('Expires In', 'pmpro'); ?>:</label></th>
                    <td>
                        <input id="expiration_number" name="expiration_number" type="text" size="10" value="<?php
                        echo esc_attr($level->expiration_number); ?>"/>
            ...
            <?php
            do_action("pmpro_membership_level_after_other_settings"); ?>

            <h3 class="topborder"><?php
                _e('Content Settings', 'pmpro'); ?></h3>
            <table class="form-table">
                <tbody>
                <tr>
                    <th scope="row" valign="top"><label><?php
                            _e('Categories', 'pmpro'); ?>:</label></th>
                    <td>
                        <?php
            ...
            <p class="submit topborder">
                ...
                _e('Cancel', 'pmpro'); ?>" onclick="location.href='<?php
                echo get_admin_url(null, '/admin.php?page=pmpro-membershiplevels') ?>';"/>
            </p>
        </form>
    </div>
    ...
    <h2 class="alignleft"><?php
        _e('Membership Levels', 'pmpro'); ?> <a href="admin.php?page=pmpro-membershiplevels&edit=-1"
                                                class="add-new-h2"><?php
            _e('Add New Level', 'pmpro'); ?></a></h2>
    <form id="posts-filter" method="get" action="">
        <p class="search-box">
            <label class="screen-reader-text" for="post-search-input"><?php
                _e('Search Levels', 'pmpro'); ?>:</label>
            <input type="hidden" name="page" value="pmpro-membershiplevels"/>
            <input id="post-search-input" type="text" value="<?php
            echo esc_attr($s); ?>" name="s" size="30"/>
            <input class="button" type="submit" value="<?php
            _e('Search Levels', 'pmpro'); ?>" id="search-submit"/>
        </p>
    </form>
    ...
    if (empty($_REQUEST['s']) && count($reordered_levels) > 1) { ?>
        <br class="clear"/>
        <p><?php
            _e('Drag and drop membership levels to reorder them on the Levels page.', 'pmpro'); ?></p>
        <?php
    } ?>
    ...
    <table class="widefat membership-levels">
        ...
        <tr>
            ...
            <th><?php
                _e('Name', 'pmpro'); ?></th>
            <th><?php
                _e('Billing Details', 'pmpro'); ?></th>
            <th><?php
                _e('Expiration', 'pmpro'); ?></th>
            <th><?php
                _e('Allow Signups', 'pmpro'); ?></th>
            <th></th>
        </tr>
        </thead>
        <tbody>
        ...
        foreach ($reordered_levels as $level) {
            ?>
            <tr class="<?php
            if ($count++ % 2 == 1) { ?>alternate<?php
            } ?> <?php
            if (!$level->allow_signups) { ?>pmpro_gray<?php
            } ?> <?php
            if (!pmpro_checkLevelForStripeCompatibility($level) || !pmpro_checkLevelForBraintreeCompatibility(
                    $level
                ) || !pmpro_checkLevelForPayflowCompatibility($level) || !pmpro_checkLevelForTwoCheckoutCompatibility(
                    $level
                )) { ?>pmpro_error<?php
            } ?>">
                <td><?php
                    echo $level->id ?></td>
                <td class="level_name"><a href="admin.php?page=pmpro-membershiplevels&edit=<?php
                    echo $level->id ?>"><?php
                        echo $level->name ?></a></td>
                <td>
                    <?php
                ...
                <td>
                    ...
                    if (!pmpro_isLevelExpiring($level)) { ?>
                        ...
                        <?php
                        _e('After', 'pmpro'); ?><?php
                        echo $level->expiration_number ?><?php
                        echo sornot($level->expiration_period, $level->expiration_number) ?>
                        <?php
                    } ?>
                </td>
                <td><?php
                    if ($level->allow_signups) { ?><a href="<?php
                    echo pmpro_url("checkout", "?level=" . $level->id); ?>"><?php
                        _e('Yes', 'pmpro'); ?></a><?php
                    } else { ?><?php
                        _e('No', 'pmpro'); ?><?php
                    } ?></td>
                ...
                <td><a title="<?php
                    _e('edit', 'pmpro'); ?>" href="admin.php?page=pmpro-membershiplevels&edit=<?php
                    echo $level->id ?>" class="button-primary"><?php
                        _e('edit', 'pmpro'); ?></a>&nbsp;<a title="<?php
                    _e('copy', 'pmpro'); ?>" href="admin.php?page=pmpro-membershiplevels&copy=<?php
                    echo $level->id ?>&edit=-1" class="button-secondary"><?php
                        _e('copy', 'pmpro'); ?></a>&nbsp;<a title="<?php
                    _e('delete', 'pmpro'); ?>" href="javascript: askfirst('<?php
                    echo str_replace(
                        "'",
                        "\'",
                        sprintf(
                            __(
                                "Are you sure you want to delete membership level %s? All subscriptions will be cancelled.",
                                "pmpro"
                            ),
                            $level->name
                        )
                    ); ?>','admin.php?page=pmpro-membershiplevels&action=delete_membership_level&deleteid=<?php
                    echo $level->id ?>'); void(0);" class="button-secondary"><?php
                        _e('delete', 'pmpro'); ?></a></td>
            </tr>
            <?php
        }
        ?>
